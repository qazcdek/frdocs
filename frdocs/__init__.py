...
# Load agency info
try:
    with open(os.path.join(data_dir, 'raw', 'agencies.msgpack'), 'rb') as f:
        agencies = load(f)
except:
    ...
    if not os.path.exists(os.path.join(download_dir, 'meta')):
        os.makedirs(os.path.join(download_dir, 'meta'))

    print('\nDownloading agency metdata')
    agencies = api.get_all_agencies()

    with open(os.path.join(data_dir, 'raw', 'agencies.msgpack'),'wb') as f:
        dump(agencies,f)
...
def load_info(frdoc_number):
    ...
    '''
    with open(os.path.join(data_dir, 'info', f'{frdoc_number}.msgpack'),'rb') as f:
        return load(f)
...
def iter_parsed(frdoc_numbers=None, missing='warn'):
    ...
    if frdoc_numbers is not None:
        ...
        files = os.listdir(os.path.join(data_dir, 'parsed'))

    for f in files:
        d = f.split('.', 1)[0]

        try:
            df = pd.read_pickle(os.path.join(data_dir, 'parsed', f))

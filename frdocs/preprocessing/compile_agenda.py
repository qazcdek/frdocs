...
def iter_raw_agenda_records(agenda_dir):
    ...
    agenda_files = [os.path.join(agenda_dir, f) for f in os.listdir(agenda_dir) if f.lower().endswith('.xml')]
...
def main(args=None):
    ...
    raw_agenda_dir = os.path.join(data_dir, 'raw', 'agenda')
    clean_agenda_dir = os.path.join(data_dir, 'agenda')
    ...
    for record in iter_raw_agenda_records(raw_agenda_dir):
        ...
        if not os.path.isdir(os.path.join(clean_agenda_dir, clean_record['publication'])):
            os.mkdir(os.path.join(clean_agenda_dir, clean_record['publication']))

        with open(os.path.join(clean_agenda_dir, clean_record['publication'], f"{clean_record['rin']}.msgpack"),'wb') as f:
            dump(clean_record,f)

...
def main(args):
    ...
    if not os.path.isdir(os.path.join(data_dir, 'info')):
        os.makedirs(os.path.join(data_dir, 'info'))
    ...
    for file in tqdm(os.listdir(os.path.join(data_dir, 'raw', 'meta'))):
        with open(os.path.join(data_dir, 'raw', 'meta', file), 'rb') as f_read:
            ...
            for record in records:
                ...
                with open(os.path.join(data_dir, 'info', f"{record['frdoc_number']}.msgpack"), 'wb') as f_write:
                    dump(record,f_write)
